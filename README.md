...
Testing is intentionally minimal: plain `node:assert` executed through `vite-node`, with the test file path hardcoded in the `test` script. Adding a new test file means updating that script.
...
app/
  ...
  routes.ts                      Route table (currently the index route only)
  ...
  lib/
    config.ts                    Storage keys, site URL, Theme type
...
Plus `public/robots.txt` and `public/sitemap.xml`, both pointing at `https://vmaker.xmtlz.dev`.

> ⚠️ There is **no `og:image` yet**. A 1200×630 bitmap is still missing, which is why `twitter:card` is `summary` rather than `summary_large_image`.
...
- **`og:image` is missing** — social previews fall back to a text card.
...
- **Only one route exists.** There is no project detail page and no admin panel, although the original wireframe (`docs/base.png`) sketched both, along with nested ("prefix") routes.
...
测试刻意保持轻量：用裸 `node:assert` 通过 `vite-node` 执行，测试文件路径写死在 `test` 脚本里。新增测试文件需要同步修改该脚本。
...
app/
  ...
  lib/
    config.ts                    storage key、站点 URL、Theme 类型
...
另有 `public/robots.txt` 与 `public/sitemap.xml`，均指向 `https://vmaker.xmtlz.dev`。

> ⚠️ 目前**还没有 `og:image`**。缺少一张 1200×630 的位图，所以 `twitter:card` 用的是 `summary` 而不是 `summary_large_image`。
...
- **目前只有一条路由。** 没有项目详情页，也没有后台管理页——尽管最初的线框图（`docs/base.png`）把两者以及「前缀子路由」都画了出来。
